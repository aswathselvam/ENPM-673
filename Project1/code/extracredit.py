...
if __name__ == '__main__':
    ...
    Parser.add_argument('--savepath', default='../outputs/ExtraCredit/', help='Template Path , Default: ../outputs/ExtraCredit/')
    args = Parser.parse_args()
    SavePath = args.savepath

    path = "../data/cityscape1.png"
    image = cv2.imread(path)
    # image = cv2.resize(image,(img_height,img_width))
    edges = cv2.Canny(image,50,50)
    plots = Plot(2,1)
    plots.set(edges,"Canny Edge - Threshold 50")
    edges100 = cv2.Canny(image,100,100)
    plots.set(edges100,"Canny Edge - Threshold 100")
    plots.save(SavePath,'CannyEdge.png')
    ...
    # # input = torch.tensor([[ image[:,:,0], image[:,:,1], image[:,:,2] ]])
    ...
    # model.load_state_dict(torch.load("10_model.pth",
    #                                  map_location=device))
    # model.eval()
    # output = model(input)
    # # print("Output's shape: ",output)

    # print(f"output shapes: {[t.shape for t in output]}")
    
    # out = output[0].detach().cpu()
    # # plots.set(out[0][0],"DexiNed Prediction")
    # print("Out's shape: ",out.shape)
    # cv2.imshow("Prediction",np.uint8(out[0][0])*100)
    # cv2.waitKey(10000)
